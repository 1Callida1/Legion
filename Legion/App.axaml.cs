            ...
            if (ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
            {
                ...
                if (context.Database.CanConnect())
                {
                    ...
                    if (!context.Users.Any())
                    {
                        ...
                        await context.ContractTypes.AddAsync(new ContractType() { ContractIdFormat = "id-E/yy", Bet = 2, Formula = "p*(1+x)^m", Period = 6, TypeName = "Накопительный Е", CanAddMoney = true, ReferalBet = 2 });
                        await context.ContractTypes.AddAsync(new ContractType() { ContractIdFormat = "id/yy", Bet = 2, Formula = "p*(1+x)^m", Period = 6, TypeName = "Накопительный", CanAddMoney = true, ReferalBet = 2 });
                        await context.ContractTypes.AddAsync(new ContractType() { ContractIdFormat = "id/yy", Bet = 4, Formula = "x*p", Period = 12, TypeName = "Инвестиционный", CanAddMoney = false, ReferalBet = 3 });
                        await context.ContractTypes.AddAsync(new ContractType() { ContractIdFormat = "id-36/yy", Bet = 6, Formula = "x*p", Period = 36, TypeName = "Трехгодовой", CanAddMoney = false, NextYearBetCoef = 0.5f, ReferalBet = 3 });
                        await context.ContractTypes.AddAsync(new ContractType() { ContractIdFormat = "id-18/yy", Bet = 7, Formula = "x*p", Period = 18, TypeName = "Доходный", CanAddMoney = false, ReferalBet = 3 });
                        await context.ContractTypes.AddAsync(new ContractType() { ContractIdFormat = "id/yy", Bet = 3, Formula = "x*p", Period = 24, TypeName = "ТАНАКА инвестиционный", CanAddMoney = false, NextYearBetCoef = 1, ReferalBet = 2 });
                        await context.ContractTypes.AddAsync(new ContractType() { ContractIdFormat = "idK/yy", Bet = 3, Formula = "p*(1+x)^m", Period = 24, TypeName = "ТАНАКА накопительный", CanAddMoney = false, NextYearBetCoef = 1, ReferalBet = 2 });
